...
class ZigDocsExtractor:
    ...
    def _get_module_docs(self, tree, code) -> list:
    ...
    def _get_functions(self, tree, code) -> list:
            ...
            if node.type == "function_declaration":
                ...
                if fn_name:
                    functions.append({
                        "name": fn_name,
                        "docstring": "\n".join(doc_comments),
                        "signature": self._get_node_text(node, code).split("{")[0].strip()
                    })
        
        return functions

    def _get_constants(self, tree, code) -> list:
        """Extract constants with /// docs"""
        constants = []
        root = tree.root_node
        
        for node in root.children:
            if node.type == "variable_declaration" and "struct" not in self._get_node_text(node, code):
    ...
    def _is_import(self, node, code) -> bool:
    ...
    def _get_structures(self, tree, code) -> list:
